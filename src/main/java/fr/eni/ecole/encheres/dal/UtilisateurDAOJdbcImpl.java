...
private static final String SELECT_ALL_UTILISATEUR = "SELECT no_utilisateur, pseudo, nom, prenom, email, telephone, rue, code_postal, ville, mot_de_passe, credit, administrateur FROM UTILISATEUR";
private static final String DELETE_UTILISATEUR_BY_ID = "DELETE UTILISATEUR FROM UTILISATEUR WHERE no_utilisateur=?";
private static final String UPDATE_UTILISATEUR = "UPDATE UTILISATEUR SET pseudo=?, nom=?, prenom=?, email=?, telephone=?, rue=?, code_postal=?, ville=?, mot_de_passe=?, credit=?, administrateur=? "
		...
		try {
			(Connection cnx = ConnectionProvider.getConnection()){
				...
				while (rs.next()) {
					...
					throw new DALException("l'utilisateur est null")
				}
		} catch (SQLException e) {
